...
class IA2C:
    ...
    def backward(self, Rends, dt, summary_writer=None, global_step=None):
        # Rends: R,  dt = self.env.T - self.cur_step ??
        self.optimizer.zero_grad()
        for i in range(self.n_agent):
            obs, nas, acts, dones, Rs, Advs = self.trans_buffer[i].sample_transition(Rends[i], dt) # sample完会reset. 
            if i == 0:
                self.policy[i].backward(obs, nas, acts, dones, Rs, Advs,
                                        self.e_coef, self.v_coef,
                                        summary_writer=summary_writer, global_step=global_step) # 只记录agent0 ??
            else:
                self.policy[i].backward(obs, nas, acts, dones, Rs, Advs,
                                        self.e_coef, self.v_coef)
    ...
    def _init_trans_buffer(self, gamma, distance_mask, coop_gamma):
        ...
        for i in range(self.n_agent):
            # init replay buffer
            self.trans_buffer.append(OnPolicyBuffer(gamma, coop_gamma, distance_mask[i])) # here
