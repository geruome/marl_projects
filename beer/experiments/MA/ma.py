...
class Env:
    def __init__(self, num_firms, p, h, c, initial_inventory, poisson_lambda=10, max_steps=100):
        ...
        # 初始化库存
        self.inventory = np.full(num_firms, initial_inventory)
        # 初始化订单量
        self.orders = np.zeros(num_firms)
        # 初始化已满足的需求量
        self.satisfied_demand = np.zeros(num_firms)
        # 记录当前步数
        self.current_step = 0
        # 标记episode是否结束
        self.done = False
    ...
    def reset(self):
        ...
        重置环境状态。
        """
        self.inventory = np.full(self.num_firms, self.initial_inventory)
        self.orders = np.zeros(self.num_firms)
        self.satisfied_demand = np.zeros(self.num_firms)
        self.current_step = 0
        self.done = False
    ...
    def _get_observation(self):
        ...
        """
        return np.concatenate([x.reshape(-1, 1) for x in [self.orders, self.satisfied_demand, self.inventory]], axis=1)
    ...
    def _generate_demand(self):
        ...
        """
        demand = np.zeros(self.num_firms)
        for i in range(self.num_firms):
            if i == 0:
                ...
                # 上游企业的需求等于下游企业的订单量
                demand[i] = self.orders[i - 1].item()  # d_{i+1,t} = q_{it}
        return demand
    ...
    def step(self, actions):
        ...
        :param actions: 每个企业的订单量 (shape: num_firms)，即每个智能体的行动
        ...
        # 更新库存
        self.inventory = self.inventory + self.orders - self.satisfied_demand
        # for i in range(self.num_firms):
        ...
        loss_sales = np.zeros(self.num_firms)  # 损失销售费用
...
def train_dqn(env: Env, agents: List[DQNAgent], num_episodes=1000, max_t=100, eps_start=1.0, eps_end=0.01, eps_decay=0.995):
    """
    训练DQN智能体
    ...
    for i_episode in tqdm(range(1, num_episodes+1)):
        state = env.reset()
        epi_score = np.zeros(env.num_firms)

        for t in range(env.max_steps):
            actions = np.zeros(env.num_firms)
            for firm_id in range(env.num_firms):
                firm_state = state[firm_id]
                action = agents[firm_id].act(firm_state, epsilon=eps)
            ...
            for firm_id in range(env.num_firms):
                agents[firm_id].step(state[firm_id].reshape(1, -1), actions[firm_id], rewards[firm_id], next_state[firm_id], done)
        ...
        # 输出进度
        if i_episode % 100 == 0:
            print(f'Episode {i_episode}/{num_episodes} | Average Score: {np.mean(scores[-100:], axis=0)} | Epsilon: {eps:.4f}')

        # 每隔一定episode保存模型
        if i_episode % 1000 == 0:
            for firm_id in range(env.num_firms):
                agents[firm_id].save(f'models/dqn_agent_firm_{firm_id}_episode_{i_episode}.pth')
...
def test_agent(env: Env, agent, num_episodes=10):
    ...
    """
    scores = [[] for _ in range(env.num_firms)]
    inventory_history = [[] for _ in range(env.num_firms)]
    orders_history = [[] for _ in range(env.num_firms)]
    demand_history = [[] for _ in range(env.num_firms)]
    satisfied_demand_history = [[] for _ in range(env.num_firms)]
    
    for i_episode in tqdm(range(1, num_episodes+1)):
        state = env.reset()
        for firm_id in range(env.num_firms):
            inventory_history[firm_id].append([])
            orders_history[firm_id].append([])
            demand_history[firm_id].append([])
            satisfied_demand_history[firm_id].append([])
            scores[firm_id].append(0)
        ...
        for t in range(env.max_steps):
            ...
            for firm_id in range(env.num_firms):            
                inventory_history[firm_id][-1].append(env.inventory[firm_id])
                orders_history[firm_id][-1].append(actions[firm_id])
                demand_history[firm_id][-1].append(env.demand[firm_id])
                satisfied_demand_history[firm_id][-1].append(env.satisfied_demand[firm_id])
                scores[firm_id][-1] += rewards[firm_id]
    ...
    print(f"Average score: {np.mean(scores, axis=1)}, Std: {np.std(scores, axis=1)}")
...
def plot_training_results(rewards, window_size=100):
    ...
    :param rewards: num_episodes * episode_len * 3 的列表
    :param window_size: 移动平均窗口大小
    """
    rewards = np.array(rewards)  # 转为 ndarray，方便处理
    num_episodes, num_companies = rewards.shape

    # shape: num_episodes × num_companies

    # 移动平均函数
    def moving_average(data, window_size):
        return [np.mean(data[max(0, i-window_size):i+1]) for i in range(len(data))]

    fig, axs = plt.subplots(3, 1, figsize=(10, 12))  # 三个子图

    for i in range(num_companies):
        scores = rewards[:, i]
        avg_scores = moving_average(scores, window_size)
        
        axs[i].plot(np.arange(len(scores)), scores, alpha=0.3, label='原始奖励')
        axs[i].plot(np.arange(len(avg_scores)), avg_scores, label=f'移动平均（窗口={window_size}）')
        axs[i].set_title(f'公司 {i+1} 的训练奖励')
        axs[i].set_xlabel('Episode')
        axs[i].set_ylabel('奖励')
        axs[i].set_ylim(bottom=-500)
        axs[i].legend()
    
    plt.tight_layout()
    plt.savefig('figures/training_rewards_per_company.png')
    plt.close()


def plot_test_results(n_scores, n_inventory_history, n_orders_history, n_demand_history, n_satisfied_demand_history):
    firm_id = 0

    for scores, inventory_history, orders_history, demand_history, satisfied_demand_history in zip(n_scores, n_inventory_history, n_orders_history, n_demand_history, n_satisfied_demand_history):
        ...
        plt.tight_layout()
        plt.savefig(f'figures/test_firm_{firm_id}.png')
        plt.close()
...
if __name__ == "__main__":
    ...
    state_size = num_firms  # 每个企业的状态维度：订单、满足的需求和库存
    action_size = 20  # 假设最大订单量为20
    
    agents = [DQNAgent(state_size=state_size, action_size=action_size, firm_id=firm_id, max_order=action_size) for firm_id in range(num_firms)]

    for firm_id, agent in enumerate(agents):
        weight_path = f'D:\Pku\classes\Multi-agent\projects\\beer\experiments\MA_0.05\models\dqn_agent_firm_{firm_id}_episode_5000.pth'
        agent.load(weight_path)

    # scores = train_dqn(env, agents, num_episodes=5000, max_t=max_steps, eps_start=1.0, eps_end=0.05, eps_decay=0.999)
    
    # plot_training_results(scores)

    test_scores, inventory_history, orders_history, demand_history, satisfied_demand_history = test_agent(env, agents, num_episodes=1000)
